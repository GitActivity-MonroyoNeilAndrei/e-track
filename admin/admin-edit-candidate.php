...
if (isset($_POST['edit-candidate'])) {
  ...
  if(is_uploaded_file($_FILES['candidate-image']['tmp_name'])) {
    $admin->updateImage('candidate-image', 'candidate', 'photo_url', '../uploads/');
  }
  
  header("location: admin-election.php?editCandidateSuccess&activeStudentOrg=" . User::returnValueGet('studentOrg'));
}
...
<body style="background-color: #EEEEEE;">
  ...
  <form class="d-flex flex-column mx-auto mt-5 p-3 border rounded-3" style="max-width: 15rem; " method="post" enctype="multipart/form-data">
    ...
    <input class="form-control" type="text" name="partylist" value="<?php echo $partylist; ?>" required>
