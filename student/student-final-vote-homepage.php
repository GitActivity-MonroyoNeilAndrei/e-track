...
$result = $student->selectDistinct('candidate', 'partylist', ['org_name' => User::returnValueGet('can_vote')]);
$k = 0;
while ($row = mysqli_fetch_assoc($result)) {
  $partylists[$k] = $row['partylist'];
  $k++;
}
  ...
  <div class="body">
    ...
    <form class="page-content d-flex flex-column p-3" method="post">
      ...
      <div style="  display: grid; grid-template-columns: repeat(<?php echo count($partylists); ?>, 1fr); grid-template-rows: auto;">
        ...
        ?>
          <div class="candidate-position border border-dark fw-bold" style="background-color: rgb(20, 120, 120);"><?php echo $partylist; ?></div>
